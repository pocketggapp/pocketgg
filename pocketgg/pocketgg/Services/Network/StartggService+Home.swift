...
extension StartggService {
  ...
  static func convertTournamentNodes(_ nodes: [TournamentNode]) -> [Tournament] {
    return nodes.compactMap {
      ...
      let start = DateFormatter.dateFromTimestamp($0.startAt)?.formatted(date: .abbreviated, time: .omitted) ?? ""
      let end = DateFormatter.dateFromTimestamp($0.endAt)?.formatted(date: .abbreviated, time: .omitted) ?? ""
