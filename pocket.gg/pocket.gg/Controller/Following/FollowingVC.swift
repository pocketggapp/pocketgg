...
final class FollowingVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if TO.customPrefix != nil || TO.customName != nil {
            attributedText = SetUtilities.getAttributedEntrantText(Entrant(id: nil, name: TO.customName, teamName: TO.customPrefix),
                                                                   bold: false, size: cell.textLabel?.font.pointSize ?? 10,
                                                                   teamNameLength: TO.customPrefix?.count)
        } else {
            attributedText = SetUtilities.getAttributedEntrantText(Entrant(id: nil, name: TO.name, teamName: TO.prefix),
                                                                   bold: false, size: cell.textLabel?.font.pointSize ?? 10,
