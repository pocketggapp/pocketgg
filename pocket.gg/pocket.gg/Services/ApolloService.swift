...
class CustomInterceptor: ApolloInterceptor {
  func interceptAsync<Operation: GraphQLOperation>(
    ...
    completion: @escaping (Result<GraphQLResult<Operation.Data>, Error>) -> Void) {
      var authToken = ""
      // TODO: OAuth - Remove when fully migrate to OAuth
      if false {
        do {
          authToken = try KeychainService.getToken(.accessToken)
        } catch {
          print(error)
        }
      } else {
        authToken = UserDefaults.standard.string(forKey: k.UserDefaults.authToken) ?? ""
      }
